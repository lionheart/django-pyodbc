...
class SQLCompiler(compiler.SQLCompiler):
    ...
    def as_sql(self, with_limits=True, with_col_aliases=False):
        ...
        ordering = self.get_ordering()[0]
        if ordering and self.connection.ops.sql_server_ver >= 2012 and not self.connection.ops.is_db2:
            ...
            if self.query.high_mark:
                ...
                sql = "%s FETCH NEXT %s ROWS ONLY;" % (sql, next_count)
            return sql, fields
        ...
        f = []
        for x in outer_fields.split(','):# save original formats
            i = x.upper().find(' AS ')
            if i != -1:
                x = x[i+4:]
            if x.find('.') != -1:
